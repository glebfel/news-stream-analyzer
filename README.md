...
- httpx — единый async HTTP‑клиент во всех сервисах; Telethon — Telegram MTProto
...
- Wikidata `wbsearchentities` (Entity Linking, top‑1 QID) и `wbgetentities` (координаты) с кешем в Redis
- OpenSearch 2.15, Neo4j 5.22 community, Redis 7.4
- FastAPI — API, Streamlit + Plotly + folium — UI
...
Эндпоинты api_gateway (см. также `/docs` со swagger):

- `GET /search?q=...&size=...&source=vk|telegram` — полнотекстовый поиск по `raw_posts`;
- `GET /latest?size=...` — N последних публикаций (используется лентой дашборда);
- `GET /suggest?q=...&size=...` — автокомплит по сущностям (prefix‑match);
- `GET /top_entities?etype=PER|ORG|LOC&size=...` — топ упоминаемых сущностей;
- `GET /entity/{wikidata_id}` — упоминания конкретной сущности;
- `GET /sankey?channels=...&per_channel=...&etype=...` — связи «сообщество → сущность»;
- `GET /locations?size=...` — топ LOC‑сущностей с координатами из Wikidata;
- `GET /graph/subgraph?entity=...&limit=...` — подграф вокруг сущности из Neo4j;
- `GET /stats`, `GET /health`, `GET /metrics`.

## Веб‑интерфейс (Streamlit)

Семь вкладок:

- **Лента** — посты обновляются каждые 10 секунд через `st.fragment(run_every=10)`;
- **Поиск** — полнотекст с автокомплитом, фильтр по источнику;
- **Статистика** — счётчики, разбивка по источникам, тональности, динамика по дням;
- **Сущности** — топ персон/организаций/локаций;
- **Граф** — подграф связей (NetworkX layout, Plotly hover, размер узла = `mention_count`);
- **Источники** — Sankey «сообщество → сущность»;
- **Карта** — folium с маркерами LOC по координатам Wikidata.
...
CI: GitHub Actions с матричной сборкой образов и публикацией в GHCR. Деплой по SSH на Yandex Cloud (2 vCPU / 8 ГБ / 50 ГБ SSD): тянутся свежие теги, `docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d`. Caddy на хосте проксирует HTTPS на api_gateway/dashboard и автоматически обновляет сертификаты Let's Encrypt по `*.nip.io`.
