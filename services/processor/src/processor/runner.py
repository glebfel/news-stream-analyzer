...
async def main() -> None:
    ...
    start_metrics_server(settings.metrics_port, service="processor")
    ...
    deduper = Deduper(threshold=0.85, redis_url=settings.redis_url)
    pipeline = ProcessorPipeline(stream_repo, posts_repo, Normalizer(), deduper)
